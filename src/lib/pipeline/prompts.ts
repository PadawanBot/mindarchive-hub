...
export function buildPrompt(step: PipelineStep, ctx: PromptContext): PromptData | null {
  switch (step) {
    ...
    case "timing_sync": {
      ...
      const wpm = ctx.format?.wpm || 145;
      return {
        system: "You are a video timing engineer. Map each visual asset to precise narration timestamps based on word count and WPM. The voiceover MP3 is the production clock. Output as JSON array with fields: section, start_time_seconds, end_time_seconds, word_count, visual_asset_id, transition_in, transition_out, notes.",
